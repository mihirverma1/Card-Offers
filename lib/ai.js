...
// Social / forum / video / news domains — excluded from the heuristic fallback.
const NOISE_DOMAINS =
  /(instagram\.com|facebook\.com|twitter\.com|x\.com|reddit\.com|youtube\.com|youtu\.be|quora\.com|linkedin\.com|pinterest\.|threads\.net|scribd\.com|timesofindia|hindustantimes|ndtv\.com|livemint|economictimes|moneycontrol)/i;
...
function parseJsonArray(text) {
  ...
  for (const c of candidates) {
    try {
      ...
      if (parsed && Array.isArray(parsed.offers)) return parsed.offers; // { "offers": [...] }
    } catch {
      /* try next candidate */
    }
  }
...
async function geminiFetch(sel) {
  ...
  const cat = sel.category
    ? ` Prioritise ${sel.category} offers, but also list other notable current offers on the card.`
    : "";
  const prompt = `You are a meticulous credit-card offers researcher. Using live Google Search, find EVERY current, active offer and benefit on the "${sel.bank} ${sel.variant}" credit card (${sel.network} network), accurate as of ${monthYear}.${cat}

Search thoroughly across ALL of these source types (run multiple searches):
- the bank's own official credit-card offers / benefits page for this exact card,
- the ${sel.network} network offers page,
- credit-card specialist sites such as CardExpert, CardInsider, Technofino, Paisabazaar, BankBazaar and LiveFromALounge,
- merchant offer pages (Amazon, Flipkart, Myntra, Swiggy, Zomato, MakeMyTrip, BookMyShow, etc.).

Include EVERY offer type that exists for this card: instant discounts, cashback, accelerated/bonus reward points, welcome benefits, milestone benefits, annual-fee waiver, airport-lounge access, dining programmes, movie-ticket offers, fuel surcharge waiver, UPI offers, and limited-period merchant deals.

For EACH offer, report on its own line block:
- Title
- Exactly what you get (use concrete numbers: % / ₹ / reward points)
- Merchant (or "All merchants")
- Minimum spend
- Maximum cap
- Eligibility
- How to avail
- Key terms & conditions
- Valid from / Valid till (dates if stated)
- Source URL (the exact page you found it on)

Rules: only include REAL offers you actually find, each with a source URL. Ignore expired deals and generic "apply now" marketing. Be exhaustive and specific — list as many genuine current offers as you can find, best deal first.`;
  ...
  if (GEMINI_API_KEY && OPENROUTER_API_KEY) {
    try {
      ...
      if (!findings || !findings.trim()) {
        console.error("geminiFetch returned empty text");
      } else {
        const offers = await llamaFormat(sel, findings);
        console.error(`Gemini+Llama: fetched ${findings.length} chars, formatted ${offers.length} offers`);
