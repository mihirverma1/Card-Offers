...
${context}`;
  ...
  for (const model of models) {
    ...
    // Retry only transient rate limits (429) once; on overload/unavailable
    // (503) or any other error, move straight to the next model to stay fast.
    if (res.status === 429) {
...
export async function aiSearch(sel) {
  ...
  pages = pages.slice(0, 6); // cap work sent to the extractor to stay within time limits
