...
export async function aiSearch(sel) {
  ...
  // Search the specialist/category media AND the official providers separately,
  // so results aren't dominated by one bank's own apply/T&C pages.
  ...
  // Widen (still excluding news/social) only if we came up short.
  ...
  // De-duplicate by URL and keep only allowed sources.
  const seen = new Set();
  pages = pages.filter((p) => {
    if (!allowedSource(p) || seen.has(p.url)) return false;
    seen.add(p.url);
    return true;
  });
